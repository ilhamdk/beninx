  ...
  return (
    <section className="grid w-full grid-cols-1 gap-2 md:grid-cols-2" id="home">
      <div className="flex flex-col items-start justify-center flex-1 gap-6 py-2 lg:-mt-40">
        <div className="flex items-center justify-center gap-2 px-2 py-1 bg-orange-100 rounded-full">
        ...
        <p className="text-[1.5rem] md:text-[3.5rem] font-bold tracking-wide text-headingColor">
          ...
          <span className="text-orange-600 text-[2rem] md:text-[3rem]">
            {" "}
            Evil Cycles
          </span>
        </p>
      ...
      <div className="relative flex items-center flex-1 py-2">
        <img src={HeroBg} className="w-full ml-auto h-420 lg:w-auto lg:h-650" alt="hero-bg" />
        <div className="absolute top-0 left-0 flex flex-wrap items-center justify-center w-full h-full gap-4 py-12 lg:-mt-6 lg:px-8">
          {heroData && heroData.map(n => (
            <div key={n.id} className="flex flex-col items-center justify-center p-4 drop-shadow-lg rounded-3xl bg-cardOverlay backdrop-blur-md lg:w-190">
            <img src={n.imageSrc} alt="i1" className="w-20 -mt-10 lg:w-40 lg:-mt-20" />
            <p className="mt-1 text-base font-semibold lg:text-xl text-textColor">{n.name}</p>
            <p className="lg:my-3 text-[12px] my-1 lg:text-sm font-semibold text-center text-lighttextGray">{n.decp}</p>
