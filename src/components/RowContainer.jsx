    ...
    <div ref={rowContainer} className={`w-full my-12 gap-3 scroll-smooth flex items-center ${flag ? "overflow-x-scroll scrollbar-none" : "overflow-x-hidden flex-wrap"}`}>
      {data && data.map(item => (
        ...
        <div className='flex items-center justify-between w-full'>
          ...
          className='object-contain w-40 -mt-8 h-36' alt="" />
          <motion.div whileTap={{ scale: 0.75 }} className="flex items-center justify-center w-8 h-8 bg-red-600 rounded-full cursor-pointer drop-shadow-2xl hover:shadow-md">
